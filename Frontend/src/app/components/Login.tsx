...
export default function Login() {
  ...
  const doLogin = async (e: string, p: string) => {
    ...
    try {
      ...
      // data = { accessToken: '...', user: { id, email, role, name } }
      if (!data?.accessToken || !data?.user) {
        setError('Unexpected response from server. Please try again.');
        return;
      }
      setAuth(data.user, data.accessToken);
      // App.tsx will re-render and show dashboard once isAuthenticated = true
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div style={{ position: 'fixed', top: '-10%', right: '-5%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(99,102,241,0.15) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'fixed', bottom: '-10%', left: '-5%', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(139,92,246,0.12) 0%, transparent 70%)', pointerEvents: 'none' }} />

      <div className="w-full max-w-md relative">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold" style={{ letterSpacing: '-0.02em' }}>
            <span style={{ color: '#EF4444' }}>v</span>
            <span style={{ color: '#1E293B' }}>Think</span>
          </h1>
        </div>
        ...
        <div className="rounded-2xl p-8 shadow-xl" style={{ background: '#FFFFFF', border: '1px solid rgba(99,102,241,0.1)' }}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-xl text-sm text-slate-900 placeholder-slate-400 outline-none"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onKeyDown={e => e.key === 'Enter' && doLogin(email, password)}
                  className="w-full pl-10 pr-10 py-3 rounded-xl text-sm text-slate-900 placeholder-slate-400 outline-none"
                  style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0' }}
                  onFocus={e => (e.currentTarget.style.borderColor = '#6366F1')}
                  onBlur={e  => (e.currentTarget.style.borderColor = '#E2E8F0')}
                />
            ...
            <div className="flex items-center justify-between">
              ...
              <button type="button" className="text-sm font-semibold" style={{ color: '#6366F1' }}>
                Forgot password?
              </button>
            </div>
            ...
            <button
              ...
              style={{
                background:  loading || !email || !password ? '#A5B4FC' : 'linear-gradient(135deg, #6366F1 0%, #7C3AED 100%)',
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {DEMO_USERS.map(u => (
              ...
              >
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0" style={{ background: u.color }}>
                  {u.name[0]}
                </div>
